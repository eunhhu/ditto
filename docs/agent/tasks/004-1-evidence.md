...
Task 004.2 later amended repair-budget and timestamp-projection behavior; see
[its verification evidence](004-2-evidence.md). The identities below describe
the reviewed Task 004.1 tree.
...
| No unbounded document retention | Context uses bounded candidate materialization, then constructs, charges, scores or embeds, and drops one document at a time while retaining top-K ranked nodes plus bounded exclusion metadata. Capability streams documents while retaining top-K roots. | `maximum_size_ten_thousand_candidate_generator_stops_at_the_shared_byte_budget` lazily generates 10,000 maximum-size context candidates, reaches the typed candidate-byte ceiling, and proves that document, lexical, and provider work remains zero. The capability top-K test retains one root from 1,000 eligible manifests without retaining retrieval documents. |
| Lifecycle-active capacity | Task 004.1 projection schema 2 filtered superseded, disputed, not-yet-valid, and expired context rows before the active candidate limit; Task 004.2 schema 3 preserves that filter with exact legacy timestamp precision. Capability manifests default to active and explicitly support active, retired, and quarantined lifecycle states. | `inactive_history_above_candidate_limit_does_not_block_active_snapshot` and `retired_and_quarantined_manifests_do_not_count_or_page_into_working_sets`. |
